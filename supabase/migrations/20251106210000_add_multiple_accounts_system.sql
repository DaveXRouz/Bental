/*
  ...
  - Adds constraints to prevent duplicate default accounts
  ...
  - Ensures users can only manage their own accounts
*/
...
INSERT INTO public.account_types (id, name, description, icon, color, category, features, allows_deposits, allows_withdrawals, allows_trading, allows_crypto, display_order) VALUES
...
ALTER TABLE public.accounts ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'active' CHECK (status IN ('active', 'frozen', 'closed'));
ALTER TABLE public.accounts ADD COLUMN IF NOT EXISTS closed_at TIMESTAMPTZ;

-- Add constraint to ensure only one default account per user
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'one_default_account_per_user'
  ) THEN
    ALTER TABLE public.accounts ADD CONSTRAINT one_default_account_per_user 
...
CREATE POLICY "Users can create own accounts with limit"
  ...
  WITH CHECK (
    auth.uid() = user_id 
    AND (SELECT COUNT(*) FROM public.accounts WHERE user_id = auth.uid()) < 10
  );
